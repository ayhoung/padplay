...
export function SubmitModal({ isOpen, onClose }: SubmitModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm sm:p-6" onClick={onClose}>
      <div 
        className="relative w-full max-w-2xl overflow-hidden rounded-xl bg-white shadow-2xl animate-in fade-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <h2 className="text-xl font-bold text-slate-900">Submit a tablet game</h2>
        ...
        <div className="max-h-[80vh] overflow-y-auto px-6 py-6">
          ...
          {step === "questionnaire" && (
            <form onSubmit={handleSubmit} className="space-y-6">
              {preview && (
                <section className="rounded-lg border border-slate-200 bg-white p-4 flex items-start gap-4 shadow-sm">
                  {preview.iconUrl && (
                    <Image
                      src={preview.iconUrl}
                      alt=""
              ...
              <div className="flex items-center gap-3 pt-2">
                <button
                  ...
                  className="text-sm font-medium text-slate-600 hover:text-slate-900 px-4 py-2 border border-slate-200 rounded-md bg-white hover:bg-slate-50 transition-colors"
                >
                  Back
                </button>
                <button type="submit" disabled={loading} className={primaryBtn}>
...
const primaryBtn =
  "inline-flex w-full sm:w-auto justify-center items-center gap-2 rounded-md bg-brand-600 px-5 py-2 text-sm font-medium text-white shadow-sm hover:bg-brand-700 disabled:bg-slate-400 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 transition-colors";
